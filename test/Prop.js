	...
	tests: [
		...
		},
		{
			...
			tests: [
				...
				{
					name: "Attribute reflection — settled state",
				...
				},
				{
					...
					async run ({ props, actions = [] }) {
						...
						Class.prototype.updated = function (event) {
							calls.push(
								[...event.changedProps].map(([name, old]) => ({
									name,
									old,
									value: this[name],
								})),
					...
					tests: [
						...
						},
						{
							...
							// oldValue flows through Prop.set, not the Computed subscriber.
							arg: {
								props: { v: { type: Number } },
								actions: [
									el => {
					...
					async run () {
						...
						for (let name of ["propchange", "change"]) {
							el.addEventListener(name, e =>
								events.push(`${name}/${e.detail.value}`));
						}
				...
				},
				{
					...
					async run () {
						...
						Class.prototype.propChangedCallback = function (event) {
							calls.push(`${event.name}/${event.detail.value}`);
						};
				...
				{
					name: "propschange fires after every propchange in the same drain",
					async run () {
						let Class = FakeElement.with({
							a: { type: Number, default: 0 },
							b: { type: Number, default: 0 },
						...
						el.addEventListener("propschange", () => order.push("propschange"));
				...
				{
					name: "propschange event exposes changedProps Map directly",
					async run () {
						let el = new (FakeElement.with({ v: { type: Number, default: 0 } }))();
						let seen;
						el.addEventListener("propschange", e => (seen = e.changedProps));
						el.mount();
						await flush();
