...
| `default`                | Any                                | undefined | The default value of the property. If a function, it's called with the element as `this` and re-runs whenever any props it reads change. Resolved values pass through `parse` (and `convert`, if specified), same as user-set values.                                                                               |
| `defaultProp`            | `string`                           | undefined | The name of another prop to use as the default value. The prop tracks the source automatically; setting an explicit value severs the link, and setting it back to `undefined` restores it. Ignored if `default` is also specified.                                                                                 |
...
| `convert`                | `Function`                         | undefined | A function to convert the provided value to the internally stored value. This does not override the conversion performed due to `type` but runs _after_ it. If `convert` reads other props (via `this.propName`), the conversion re-runs automatically when those props change (auto-tracked via signals).           |
